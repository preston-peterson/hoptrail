...
// TestUpdater_RestartErrorIsNotFailure pins the fix: once the binary is
// swapped + setcap'd, a restart error (typically this process being
// terminated by its own restart) must NOT be reported as a failure —
// the central confirms success via heartbeat.
func TestUpdater_RestartErrorIsNotFailure(t *testing.T) {
	fc := newFakeCentral(t)
	u, dir, _ := testUpdater(t, fc, "v0.6.1")
	u.Hooks.Restart = func(context.Context) error {
		return errors.New("signal: terminated") // the cgroup-kill race
	}
	if err := u.apply(context.Background(), cmdFor("0.7.0", updaterELF)); err != nil {
		t.Fatalf("apply returned error on restart-terminated, want nil: %v", err)
	}
	// Binary was swapped (the update materially succeeded).
	live, _ := os.ReadFile(filepath.Join(dir, "bin", "hoptrail"))
	if string(live) != string(updaterELF) {
		t.Error("binary not swapped despite successful apply")
	}
	// No failure was reported.
	if st := fc.lastStatus(); st != nil && st["state"] == "failed" {
		t.Errorf("reported failure on an apply that succeeded: %v", st)
	}
}
