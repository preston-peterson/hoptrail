...
type HeartbeatConfig struct {
	...
	// OnUpdate receives the update command when the reply carries one
	// (#22). Nil-able; the Updater's OnCommand is the production
	// wiring.
	OnUpdate func(UpdateCommand)
}
...
func beatOnce(ctx context.Context, client *Client, cfg HeartbeatConfig) (ok bool, fatal error) {
	...
	case OutcomeOK:
		...
		if reply.Update != nil && cfg.OnUpdate != nil {
			cfg.OnUpdate(*reply.Update)
		}
		return true, nil
