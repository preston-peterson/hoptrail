...
type Config struct {
	...
	// CheckpointEvery is the WAL-truncate cadence between sweeps.
	// Sized for RESILIENCE, not lock pressure: 10-minute bucketing of
	// overnight lock errors (2026-08-14) showed them clustering
	// entirely inside failed-checkpoint windows — 43 in the single
	// bucket containing the 06:05Z quiesce failure, ZERO elsewhere
	// even with the WAL at 200-300 MB under constant polling. WAL
	// size itself was never hurting anyone; the pre-gate checkpoint's
	// own starvation was. What this cadence buys is blast radius: a
	// missed truncate now costs ~10 minutes of growth (~35 MB at the
	// measured 215 MB/h) instead of a full hour's carried into the
	// next cycle (~215+ MB, observed live). It assumes the gated
	// checkpoint in storage — pre-gate, more checkpoints meant more
	// starvation windows and this setting would have made things
	// WORSE. Zero disables the extra ticks (sweeps still checkpoint).
...
func (w *Worker) Run(ctx context.Context) {
	// The banner reports the EFFECTIVE policy, not the yaml seed: the
	// DB row overrides it live (step-110), and a banner showing the
	// seed cost a 4× dataset-sizing error during the 2026-08 incident
	// review — an operator reads this line as the policy, so it must
	// be the policy.
	w.log.Info("retention: worker started",
		"retention_days", w.EffectiveDays(ctx),
		"interval", w.cfg.Interval,
		"checkpoint_every", w.cfg.CheckpointEvery)
	...
	// Separate, faster cadence for the WAL truncate alone — see the
	// Config.CheckpointEvery comment for the sizing story.
	ckptCh := make(<-chan time.Time) // nil-like: never fires when disabled
	if w.cfg.CheckpointEvery > 0 {
		ckptTicker := time.NewTicker(w.cfg.CheckpointEvery)
...
func (w *Worker) runOnce(ctx context.Context) {
	...
	// deletes is the point of maximum WAL fatness. Between sweeps the
	// CheckpointEvery ticker calls the same routine.
	w.checkpoint(ctx)
...
// checkpoint runs one blocking TRUNCATE with retries and logs the
// outcome. Always logged on success: a truncate with frames=0 is
// still a truncate (passive checkpoints do the copying — the reset is
// the part that matters), and a silent success is indistinguishable
// from no attempt when reading the journal after an incident.
