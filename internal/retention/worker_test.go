...
// The checkpoint-only ticker truncates the WAL between sweeps: with a
// long sweep Interval and a tiny CheckpointEvery, rows written after
// Run starts must still get their WAL folded away well before any
// sweep fires.
func TestWorker_CheckpointTickerTruncatesBetweenSweeps(t *testing.T) {
	dir := t.TempDir()
	...
	w := New(Config{RetentionDays: 7, Interval: time.Hour, CheckpointEvery: 25 * time.Millisecond}, store, nil)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	go w.Run(ctx)

	// Let the startup sweep finish, then generate WAL churn.
	time.Sleep(50 * time.Millisecond)
	for i := 0; i < 300; i++ {
		if _, err := store.DB().ExecContext(ctx,
			`INSERT INTO samples (target, ttl, ts, ip, rtt_us) VALUES (?, ?, ?, ?, ?)`,
			"192.0.2.9", 1, time.Now().UnixMilli(), "192.0.2.1", 100); err != nil {
			t.Fatalf("insert: %v", err)
		}
	}

	// A ticker fire (≤25ms away) must truncate the WAL to zero — no
	// writers are running, so it stays zero once reset.
	...
	t.Fatalf("wal never truncated by the checkpoint ticker (size now %d)", fi.Size())
}
