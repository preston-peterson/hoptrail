...
func TestApplyProbeResult_DestUnreachableDoesNotPolluteHopIdentity(t *testing.T) {
	...
	router := mustIP(t, "10.0.0.1")
	localBox := mustIP(t, "192.168.1.231")
	ts := time.Now()
...
func TestSnapshot_OrdersByTTL(t *testing.T) {
	...
	s.ApplyProbeResult(ProbeResult{Target: s.Target(), TTL: 3, Ts: ts, RespIP: mustIP(t, "10.0.0.3"), RTT: 3 * time.Millisecond, Reply: ReplyTimeExceeded}, 3)
	s.ApplyProbeResult(ProbeResult{Target: s.Target(), TTL: 1, Ts: ts, RespIP: mustIP(t, "10.0.0.1"), RTT: 1 * time.Millisecond, Reply: ReplyTimeExceeded}, 3)
