...
func TestEngine_RouteChangeEmittedAtThreshold(t *testing.T) {
	...
	a, _ := netip.ParseAddr("203.0.113.1")
	b, _ := netip.ParseAddr("203.0.113.2")
	ts := time.Now()
...
func TestEngine_SweepResult_AppliesEveryTTL(t *testing.T) {
	...
	r1, _ := netip.ParseAddr("203.0.113.1")
	r2, _ := netip.ParseAddr("203.0.113.2")
	r3, _ := netip.ParseAddr("203.0.113.3")
	ts := time.Now()
