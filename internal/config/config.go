...
type Config struct {
	...
	// AllowedHosts is the extra Host-header allowlist for the
	// anti-DNS-rebinding guard (step-170). Loopback, bare IP literals,
	// and single-label/.local intranet names are always accepted
	// without listing; add reverse-proxy hostnames or public FQDNs you
	// reach the UI at here. Empty is the common case (direct LAN access).
