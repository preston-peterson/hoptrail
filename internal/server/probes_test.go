...
func seedProbe(t *testing.T, f *fixture, probeID string, lastSeen time.Time) {
	t.Helper()
	if err := f.store.UpsertProbeHeartbeat(context.Background(), probeID, "v0.3.0-test", lastSeen.Add(-time.Hour), lastSeen, "192.0.2.70"); err != nil {
...
func TestHandleProbes_OutdatedFlag(t *testing.T) {
	...
	seed := func(id, version string) {
		t.Helper()
		if err := uf.store.UpsertProbeHeartbeat(context.Background(), id, version, now.Add(-time.Hour), now, "192.0.2.71"); err != nil {
