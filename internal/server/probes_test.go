...
func TestResolveProbeID_Semantics(t *testing.T) {
	...
	insertSample(t, f, 1, now, "203.0.113.1", 500) // local row
...
func TestHandleSamples_ProbeIDScopesRows(t *testing.T) {
	...
	seedProbe(t, f, "site-east-pi", time.Now())
	now := time.Now().UnixMilli()
	insertSample(t, f, 1, now-1000, "203.0.113.1", 500) // local
	...
	for _, tc := range []struct {
		...
		{"/api/samples", "203.0.113.1"},
		{"/api/samples?probe_id=site-east-pi", "198.51.100.9"},
	} {
...
func TestHandleExport_ProbeIDScoped(t *testing.T) {
	...
	if len(bundle.Samples) != 1 || bundle.Samples[0].IP == nil || *bundle.Samples[0].IP != "198.51.100.9" {
