...
type layoutJSON struct {
	...
	// Heights holds operator-dragged per-section heights in px
	// (step-164: a horizontal grab bar on each section's bottom edge —
	// the height twin of the dock splitter). Absent id = natural
	// height (the CSS default). Clamped 120-1600 by normalize.
...
func normalizeLayout(in layoutJSON) layoutJSON {
	...
	out := layoutJSON{Order: []string{}, SidePosition: "right", SideWidth: 340, Collapsed: map[string]bool{}, Heights: map[string]int{}}
	...
	for id, h := range in.Heights {
		if known[id] && h >= 120 && h <= 1600 {
			out.Heights[id] = h
		}
	}
	return out
