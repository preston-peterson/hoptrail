...
func (s *Server) handleBandwidthRun(w http.ResponseWriter, r *http.Request) {
	...
	if s.cfg.BandwidthRunner.KillSwitched() {
		// Name the mechanism: a silent 503 here would send the operator
		// hunting through config rows that are all correct.
		http.Error(w, "speedtests disabled by the "+bandwidth.EnvNoSpeedtest+" kill switch on the daemon", http.StatusServiceUnavailable)
		return
	}
