...
func (s *Server) heartbeatUpdateFor(ctx context.Context, probeID, reportedVersion string, now time.Time) *heartbeatUpdateCommand {
	...
	if pu == nil || pu.State == storage.ProbeUpdateApplied {
		return nil
	}

	// Success wins over everything, INCLUDING a prior "failed": if the
	// probe is now reporting the target version, the update demonstrably
	// worked. This self-heals the false-failure case where the
	// in-process restart terminated the probe mid-call (the restart
	// "signal: terminated" race) — the binary swapped fine and the new
	// version is live, so the truth is "applied". (base compare —
	...
	if pu.State == storage.ProbeUpdateFailed {
		return nil // terminal, and the probe isn't on the target version
	}
