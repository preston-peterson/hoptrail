...
func prodStagedVersion(ctx context.Context, path string) (string, error) {
	...
	// SECURITY (step-170, audit #11): the staged binary is not yet
	// trusted. Probe it with no inherited environment so it can't leak
	// the daemon's env, and note it carries no file capabilities of its
	// own (setcap runs only at apply) so it cannot acquire cap_net_raw
	// here. The primary fix is upstream: staging no longer execs at all
	// (audit #1) — only the operator-driven status read and apply reach
	// this, both behind the cross-origin guard.
	cmd.Env = []string{}
	out, err := cmd.Output()
	if err != nil {
...
type updateStatusResponse struct {
	...
	// Release-mode state (#11). ReleaseAvailable says whether this
	// build can talk to GitHub at all; LastCheck is nil until a check
	// (manual or background) has completed.
	ReleaseAvailable bool             `json:"release_available"`
	CheckInterval    string           `json:"check_interval"`
	LastCheck        *updateCheckJSON `json:"last_check,omitempty"`
}

// ---------- GET/PATCH /api/update ----------
...
func (s *Server) handleUpdateStatus(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPatch:
		s.handleUpdateSettingsPatch(w, r)
	default:
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
	}
}

func (s *Server) writeUpdateStatus(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	resp := updateStatusResponse{
		RunningVersion:   s.versionString(),
		Arch:             runtime.GOARCH,
		Staged:           s.stagedInfo(ctx),
		Sudoers:          s.sudoersCheck(ctx),
		ReleaseAvailable: s.cfg.ReleaseSource != nil,
		CheckInterval:    s.checkIntervalSetting(ctx),
	}
	if resp.ReleaseAvailable {
		resp.LastCheck = s.lastCheckJSON(s.releaseChecker().ReadLastCheck(ctx))
	}
	writeJSON(w, resp)
}
...
func (s *Server) handleUpdateUpload(w http.ResponseWriter, r *http.Request) {
	...
	// SECURITY (step-170, audit #1): do NOT run the just-staged binary
	// here. The version probe (which execs it) is deferred to the GET
	// status read and the apply path; staging never executes attacker-
	// influenced bytes. Return a stat-only summary.
	writeJSON(w, s.stagedInfoStatOnly())
}

// stagedInfoStatOnly returns presence/size/mtime WITHOUT executing the
// staged binary (no version probe) — used on the write path so merely
// staging a file never runs it.
func (s *Server) stagedInfoStatOnly() updateStagedJSON {
	fi, err := os.Stat(s.stagedBinPath())
	if err != nil {
		return updateStagedJSON{Present: false}
	}
	return updateStagedJSON{
		Present:    true,
		SizeBytes:  fi.Size(),
		ModifiedAt: fi.ModTime().UnixMilli(),
	}
