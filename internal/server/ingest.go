...
func (s *Server) authAgent(w http.ResponseWriter, r *http.Request) (token string, ok bool) {
	dbTokens, err := s.cfg.Store.ProbeTokenSecrets(r.Context())
	if err != nil {
		s.log.Error("ingest: probe token lookup failed", "err", err)
		http.Error(w, "token lookup failed", http.StatusInternalServerError)
		return "", false
	}
	accepted := append(dbTokens, s.cfg.AgentTokens...)
	if len(accepted) == 0 {
...
func (s *Server) handleIngestHeartbeat(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.cfg.Store.UpsertProbeHeartbeat(r.Context(), req.ProbeID, req.Version, time.UnixMilli(req.StartedAt), now, remoteIP); err != nil {
...
func checkClockSkew(tsMs int64, now time.Time) error {
	skew := time.Duration(tsMs-now.UnixMilli()) * time.Millisecond
	if skew > clockSkewBound || skew < -clockSkewBound {
		return fmt.Errorf("ts %d is %s from central's clock (bound %s) — check the agent's NTP", tsMs, skew.Round(time.Second), clockSkewBound)
	}
	return nil
}
