...
func TestIngestPath_MissingHopsStoredAsEmptyArray(t *testing.T) {
	f := newFixture(t)
	body := fmt.Sprintf(`{"probe_id":"site-east-pi","target":"8.8.8.8","ts":%d,"hop_count":0,"target_ttl":0}`, time.Now().UnixMilli())
	code, respBody := f.postIngest(t, "/api/ingest/path", testAgentToken, body)
	if code != http.StatusOK {
		t.Fatalf("status = %d, want 200; body=%s", code, respBody)
	}
...
// TestIngest_TokenBoundToProbe pins audit #10: a UI-minted token may
// only push as the probe it was minted for; the legacy yaml token stays
// unbound.
func TestIngest_TokenBoundToProbe(t *testing.T) {
	f := newFixture(t)
	// Mint a DB token bound (by name) to "site-a".
	if _, err := f.store.InsertProbeToken(context.Background(), "bound-token-aaaaaaaaaaaaaaaaaaaa", "site-a", timeNow()); err != nil {
		t.Fatalf("mint: %v", err)
	}

	hb := func(token, probeID string) int {
		body := `{"probe_id":"` + probeID + `","version":"v1","started_at":1,"targets":[],"arch":"amd64"}`
		code, _ := f.postIngest(t, "/api/ingest/heartbeat", token, body)
		return code
	}

	// Bound token as its own probe: OK.
	if code := hb("bound-token-aaaaaaaaaaaaaaaaaaaa", "site-a"); code != http.StatusOK {
		t.Errorf("bound token as site-a = %d, want 200", code)
	}
	// Bound token impersonating another probe: 403.
	if code := hb("bound-token-aaaaaaaaaaaaaaaaaaaa", "site-b"); code != http.StatusForbidden {
		t.Errorf("bound token as site-b = %d, want 403", code)
	}
	// Legacy yaml token: unbound, may act as any probe.
	if code := hb(testAgentToken, "site-b"); code != http.StatusOK {
		t.Errorf("yaml token as site-b = %d, want 200 (legacy unbound)", code)
	}
}
