...
func TestStatus_Aggregate(t *testing.T) {
	...
	// The card names the incident, not just the count.
	if len(st.Alerts.Active) != 1 ||
		st.Alerts.Active[0].Event != "probe_offline" ||
		st.Alerts.Active[0].Subject != "site-east" ||
		st.Alerts.Active[0].Since != 1 {
		t.Errorf("alerts.active = %+v", st.Alerts.Active)
	}
