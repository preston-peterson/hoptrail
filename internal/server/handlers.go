...
func (s *Server) handleTargetDelete(w http.ResponseWriter, r *http.Request, id string) {
	...
	// A removed target's loss/latency incidents would otherwise be
	// orphaned — the engine never evaluates a subject that no longer
	// exists (step-195).
	s.clearSubjectAlertStates(r.Context(), id, alert.EventTargetLoss, alert.EventLatency)
