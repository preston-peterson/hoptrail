...
// TestProbeUpdate_FailedSelfHealsOnSuccessHeartbeat pins the fix for the
// restart "signal: terminated" false-failure: a probe marked failed
// that then heartbeats on the target version is recognized as applied.
func TestProbeUpdate_FailedSelfHeals(t *testing.T) {
	uf, _, token := newProbeUpdateFixture(t)
	seedUpdatableProbe(t, uf, "site-east", "v0.5.0", "amd64")
	commandUpdate(t, uf, "site-east").Body.Close()

	// Force a (false) failure, as the restart race would.
	if err := uf.store.SetProbeUpdateState(context.Background(), "site-east", storage.ProbeUpdateFailed, "restart hoptrail-probe: signal: terminated", time.Now().UnixMilli()); err != nil {
		t.Fatal(err)
	}
	// The new binary comes up and heartbeats on the target version.
	heartbeat(t, uf, token, "site-east", "v0.6.0")
	pu, _ := uf.store.GetProbeUpdate(context.Background(), "site-east")
	if pu.State != storage.ProbeUpdateApplied {
		t.Errorf("state = %s, want applied (self-heal from false failure)", pu.State)
	}
}
