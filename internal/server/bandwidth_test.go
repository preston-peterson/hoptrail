...
func (f *fakeBandwidthRunner) KillSwitched() bool { return f.killSwitched }
...
func TestBandwidthRun_StatusCodes(t *testing.T) {
	...
	// Kill switch → 503 that NAMES the env var (not a mystery refusal).
	_, ts4 := bwFixture(t, &fakeBandwidthRunner{killSwitched: true}, bandwidth.Capability{Available: true})
	code, body := httpDo(t, "POST", ts4.URL+"/api/bandwidth/run", "")
	if code != http.StatusServiceUnavailable {
		t.Errorf("kill switch: status = %d, want 503", code)
	}
	if !strings.Contains(string(body), bandwidth.EnvNoSpeedtest) {
		t.Errorf("kill switch body = %q, want it to name %s", body, bandwidth.EnvNoSpeedtest)
	}
}
