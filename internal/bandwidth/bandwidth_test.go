...
func TestNextIntervalRun(t *testing.T) {
	...
	none := time.Time{}
	// No samples and no attempts yet → 30s out.
	if got := NextIntervalRun(nil, none, 60, now); !got.Equal(now.Add(30 * time.Second)) {
	...
	if got := NextIntervalRun(latest, none, 60, now); !got.Equal(now.Add(40 * time.Minute)) {
	...
	if got := NextIntervalRun(overdue, none, 60, now); !got.Equal(now.Add(time.Second)) {
		t.Errorf("overdue: %v", got)
	}
	// THE RUNAWAY GUARD: store failing, so the stored sample is stuck
	// 3h in the past — but a test was attempted 20m ago. The attempt
	// wins the anchor: next run is 40m out, NOT now+1s forever.
	attempted := now.Add(-20 * time.Minute)
	if got := NextIntervalRun(overdue, attempted, 60, now); !got.Equal(now.Add(40 * time.Minute)) {
		t.Errorf("store-failure anchor: %v, want attempt+interval", got)
	}
	// No samples at all (store never worked) but an attempt exists →
	// still paced by the interval, not the 30s bootstrap.
	if got := NextIntervalRun(nil, attempted, 60, now); !got.Equal(now.Add(40 * time.Minute)) {
		t.Errorf("nil latest with attempt: %v, want attempt+interval", got)
	}
}

// The out-of-band kill switch: with HOPTRAIL_NO_SPEEDTEST set, no test
// runs by any path — scheduled, catch-up, or manual — because the
// DB-backed enabled toggle is unreachable exactly when storage fails.
func TestKillSwitch_BlocksAllTests(t *testing.T) {
	t.Setenv(EnvNoSpeedtest, "1")
	store := testStore(t)
	cfg := DefaultConfig()
	cfg.Enabled = true
	ran := false
	...
	if !r.KillSwitched() {
		t.Fatal("KillSwitched() = false with the env var set")
	}
	if r.RunNow() {
		t.Error("RunNow accepted a trigger under the kill switch")
	}
	r.execute(context.Background(), cfg)
	if ran {
		t.Error("execute invoked the speedtest CLI under the kill switch")
	}

	// Run parks until ctx cancel without scheduling anything.
	...
	case <-time.After(2 * time.Second):
		t.Fatal("Run did not exit on ctx cancel under the kill switch")
	}
	if ran {
		t.Error("a speedtest ran under the kill switch")
	}
}
