...
// EnvNoSpeedtest is the out-of-band kill switch: when this environment
// variable is set (any non-empty value), NO speedtest runs — scheduled
// or manual — regardless of the bandwidth.* config rows. It exists
// because those rows live in the database: when storage is failing
// (disk full), the settings-panel toggle cannot be written, and a
// 2026-08 incident showed that leaves no way to stop a misbehaving
// scheduler short of stopping the whole daemon. Set it via a systemd
// drop-in (Environment=HOPTRAIL_NO_SPEEDTEST=1) and restart.
const EnvNoSpeedtest = "HOPTRAIL_NO_SPEEDTEST"
...
// catchupDelay is how long after startup a missed-slot catch-up test
// waits. It was 5 seconds, which phase-aligned the catch-up's sample
// store with the startup retention sweep — the one moment the
// database is guaranteed busy, and both fire on exactly the restarts
// the catch-up exists for. Five consecutive catch-up samples died to
// that collision on a live box. Four minutes clears the sweep.
const catchupDelay = 4 * time.Minute
...
type Runner struct {
	...
	cfg      Config
	inFlight bool
	// lastAttempt is when a test last STARTED in this process — set on
	// every execute regardless of outcome, so the scheduler's memory of
	// "I already tried" cannot be lost to the very store failure it
	// needs protection from (the 2026-08 runaway: a full disk made
	// RecordBandwidthSample fail, the sample marker never advanced, and
	// the catch-up path re-ran a line-saturating test every 5 seconds
	// for 13 hours). In-memory only: a restart forfeits at most one
	// extra attempt.
	lastAttempt time.Time

	// killSwitch mirrors EnvNoSpeedtest at construction time.
	killSwitch bool
...
func NewRunner(store *storage.Store, cfg Config, run CommandRunner, pause Pauser, log *slog.Logger) *Runner {
	...
	return &Runner{
		...
		now:        time.Now,
		cfg:        cfg,
		killSwitch: os.Getenv(EnvNoSpeedtest) != "",
...
// KillSwitched reports whether EnvNoSpeedtest was set at startup (the
// API surfaces this as a 503 with the variable's name, rather than a
// silently ignored trigger).
func (r *Runner) KillSwitched() bool {
	return r.killSwitch
}

// RunNow triggers a manual test. Returns false when one is already
// in flight (the API maps that to 409) or the kill switch is set.
...
func (r *Runner) Run(ctx context.Context) {
	if r.killSwitch {
		// Deliberately loud and once: the operator set this to stop
		// tests; the schedule and manual triggers are dead until the
		// variable is removed and the daemon restarted.
		r.log.Warn("bandwidth: kill switch set, no speedtests will run", "env", EnvNoSpeedtest)
		<-ctx.Done()
		return
	}
...
func (r *Runner) execute(ctx context.Context, cfg Config) {
	if r.killSwitch {
		r.log.Warn("bandwidth: test suppressed by kill switch", "env", EnvNoSpeedtest)
		return
	}
	r.mu.Lock()
	if r.inFlight {
	...
	r.inFlight = true
	// Attempt marker BEFORE the test, unconditionally: the schedule
	// must remember this attempt even if the test and the store both
	// fail (see the lastAttempt field comment).
	r.lastAttempt = r.now()
	r.mu.Unlock()
...
func (r *Runner) nextRun(ctx context.Context, cfg Config) (time.Time, bool) {
	...
	if cfg.CadenceMode == "interval" {
		...
		return NextIntervalRun(latest, attempted, cfg.IntervalMin, r.now()), true
	}
	...
	//
	// The lastAttempt guard makes catch-up AT MOST ONCE per slot per
	// process. The stored-sample check alone is not enough: this loop
	// used to reason "failed tests are stored too, so this can't loop"
	// — but when the STORE is what's failing (disk full), no row ever
	// lands, and without the in-memory guard this path re-armed a
	// full-bandwidth test every 5 seconds until the service was killed
	// (2026-08 incident, ~700 attempts/hour for 13 hours).
	if prev, ok := PrevScheduled(r.now(), cfg.ScheduledTimes, cfg.Location()); ok && attempted.Before(prev) {
		latest, err := r.store.LatestBandwidthSample(ctx)
		if err == nil && (latest == nil || time.UnixMilli(latest.Ts).Before(prev)) {
			...
			return r.now().Add(catchupDelay), true
		}
	}
...
// NextIntervalRun computes anchor + interval, clamped to no earlier
// than now+1s (a long-overdue test fires promptly, not in the past).
// The anchor is the LATER of the last stored sample and the last
// in-process attempt: anchoring on stored samples alone meant a
// failing store (disk full) froze the anchor in the past, and the
// now+1s clamp turned the cadence into a test-per-second retry loop —
// the interval-mode twin of the scheduled-mode catch-up runaway. No
// sample and no attempt yet → now+30s (the enable-toggle first-test
// promise).
func NextIntervalRun(latest *storage.BandwidthSample, lastAttempt time.Time, intervalMin int, now time.Time) time.Time {
	anchor := lastAttempt
	if latest != nil {
		if ts := time.UnixMilli(latest.Ts); ts.After(anchor) {
			anchor = ts
		}
	}
	if anchor.IsZero() {
	...
	next := anchor.Add(time.Duration(intervalMin) * time.Minute)
