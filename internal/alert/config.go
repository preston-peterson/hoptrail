...
type Config struct {
	...
	// Capacity thresholds (disk/DB growth monitor). Free disk alerts at
	// the larger of an absolute MB floor and a fraction of the volume;
	// the headroom ratio is (free+db)/projected-settled-size, so below
	// 1.0 the database is on track to fill the disk.
	DiskFreeFloorMB   int     // absolute free-disk floor in MB (0-1048576)
	DiskFreePctFloor  float64 // free-disk floor as a fraction of total (0-1)
	HeadroomThreshold float64 // alert when headroom drops below this (>=1.0)
...
func DefaultConfig() Config {
	return Config{
		...
		DiskFreeFloorMB:   1024,
		DiskFreePctFloor:  0.05,
		HeadroomThreshold: 1.2,
		CooldownS:         1800,
		RateLimitPerH:     12,
	}
}
...
func (c Config) Validate() error {
	...
	if c.DiskFreeFloorMB < 0 || c.DiskFreeFloorMB > 1048576 {
		return fmt.Errorf("disk_free_floor_mb %d out of range 0-1048576", c.DiskFreeFloorMB)
	}
	if c.DiskFreePctFloor < 0 || c.DiskFreePctFloor > 1 {
		return fmt.Errorf("disk_free_pct_floor %v out of range 0-1", c.DiskFreePctFloor)
	}
	if c.HeadroomThreshold < 1 || c.HeadroomThreshold > 100 {
		return fmt.Errorf("headroom_threshold %v out of range 1-100", c.HeadroomThreshold)
	}
...
func (c *Config) fields() map[string]struct {
	...
	return map[string]struct {
		...
		"events.capacity":      boolField(&c.EventCapacity),
		"loss_pct": {
		...
		"disk_free_floor_mb": intField(&c.DiskFreeFloorMB),
		"disk_free_pct_floor": {
			get: func() string { return strconv.FormatFloat(c.DiskFreePctFloor, 'f', -1, 64) },
			set: func(v string) bool {
				f, err := strconv.ParseFloat(v, 64)
				if err == nil {
					c.DiskFreePctFloor = f
				}
				return err == nil
			},
		},
		"headroom_threshold": {
			get: func() string { return strconv.FormatFloat(c.HeadroomThreshold, 'f', -1, 64) },
			set: func(v string) bool {
				f, err := strconv.ParseFloat(v, 64)
				if err == nil {
					c.HeadroomThreshold = f
				}
				return err == nil
			},
		},
