...
type Providers struct {
	...
	// Capacity reports disk/DB capacity standing. `active` tells the
	// provider whether an alert is already raised so it can apply
	// recovery hysteresis. Valid=false means no measurement this tick
	// (skip without disturbing any existing incident).
	Capacity func(ctx context.Context, active bool) (CapacityVerdict, error)
}

// CapacityVerdict is the capacity provider's evaluated result —
// primitives only, so the engine stays free of the capacity package
// (the closure in main.go does the measurement and threshold math).
...
func (e *Engine) Tick(ctx context.Context) error {
	...
	// ---- disk / db capacity ----
	// Subject is fixed ("disk") — one incident at a time. The provider
	// applies recovery hysteresis using the active flag, so the
	// condition is already debounced (sustain 0, like derate). A
	// measurement error is logged, not fatal: a transient statfs hiccup
	// must not take down the rest of the tick.
	if cfg.EventCapacity && e.prov.Capacity != nil {
		active := stateActive(stateOf, EventCapacity, "disk")
		if v, err := e.prov.Capacity(ctx, active); err != nil {
			e.log.Warn("alert: capacity check failed", "err", err)
		} else if v.Valid {
			e.step(ctx, cfg, now, stateOf, EventCapacity, "disk", v.Tripped, 0,
				v.AlertMsg, v.RecoverMsg, "high")
		}
	}

	// ---- orphan sweep (step-195) ----
	// A subject that disappears from monitoring entirely (probe
	// forgotten, target removed) never gets another step() call, so
	// its state row would sit "active" forever — a status card
	// alerting on a probe the operator deleted 40 days ago, with no
	// way to clear it from any UI. Only the dynamic-subject events
	// need this; derate and capacity have fixed subjects that are
	// evaluated every tick. Each event type sweeps ONLY when its
	// section above actually evaluated (same enabled+provider
	// conditions), so a disabled event or absent provider never
	// causes a false sweep, and provider errors returned early and
	// never reach here. Deliberately NOT a recovery notification —
	// nothing recovered; the subject stopped existing.
	for _, st := range stateOf {
		orphaned := false
		switch st.EventType {
		case EventProbeOffline:
			_, exists := probes[st.Subject]
			orphaned = probesEvaluated && !exists
		case EventTargetLoss:
			_, exists := targets[st.Subject]
			orphaned = cfg.EventTargetLoss && e.prov.Targets != nil && !exists
		case EventLatency:
			_, exists := targets[st.Subject]
			orphaned = cfg.EventLatency && e.prov.Targets != nil && !exists
		}
		if orphaned {
			e.clearOrphan(ctx, now, st)
		}
	}
	return nil
}

// clearOrphan deletes one orphaned incident. History gets a "cleared"
// entry (only for states that actually notified — a raising-but-
// never-fired state just goes quietly), and the in-memory cooldown
// forgets the subject so a re-added one starts fresh.
func (e *Engine) clearOrphan(ctx context.Context, now time.Time, st *storage.AlertState) {
	if st.State == "active" {
		if err := e.store.AppendAlertHistory(ctx, storage.AlertHistoryEntry{
			Ts: now.UnixMilli(), EventType: st.EventType, Subject: st.Subject, Kind: "cleared",
			Message: fmt.Sprintf("%s removed from monitoring — %s incident cleared", st.Subject, st.EventType),
		}); err != nil {
			e.log.Warn("alert: orphan history write failed", "err", err)
		}
	}
	if err := e.store.DeleteAlertState(ctx, st.EventType, st.Subject); err != nil {
		e.log.Warn("alert: orphan state delete failed", "err", err)
		return
	}
	e.mu.Lock()
	delete(e.lastRaised, st.EventType+"|"+st.Subject)
	e.mu.Unlock()
	e.log.Info("alert: cleared orphaned incident", "event", st.EventType, "subject", st.Subject)
}

// stateActive reports whether an incident is currently raised.
func stateActive(stateOf map[string]*storage.AlertState, event, subject string) bool {
	st := stateOf[event+"|"+subject]
	return st != nil && st.State == "active"
}
...
func (e *Engine) raise(ctx context.Context, cfg Config, now time.Time,
	...
	suppressed := seen && now.Sub(last) < time.Duration(cfg.CooldownS)*time.Second
	if !suppressed {
		// Record the ATTEMPT, not the delivery (2026-08 incident,
		// bug 4): when the store is failing, the persisted state row
		// never lands, so every tick re-raises the same incident as
		// new — and with this marker gated on delivery success,
		// nothing bounded the loop: 240 raise-attempts/hour from one
		// subject saturated the global rate limit, starving every
		// other alert and spamming the log hard enough to rotate the
		// journal away. Attempt-time recording caps re-raises at one
		// per cooldown per subject no matter how broken storage is.
		// The trade: a raise dropped by the rate limiter waits out
		// the cooldown instead of retrying next tick — bounded noise
		// over prompt retry, deliberately.
