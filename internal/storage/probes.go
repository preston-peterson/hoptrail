...
type Probe struct {
	...
	LastIP *string
	// Arch is the probe's reported GOARCH (step-168) — which release
	// binary a central-driven update must serve it. Nil until the
	// probe runs a version that reports it.
	Arch *string
	// Pin excludes this probe from fleet-wide updates (the individual
	// Update button still works deliberately).
	Pin bool
}
...
func (s *Store) UpsertProbeHeartbeat(ctx context.Context, probeID, version string, startedAt, lastSeenAt time.Time, lastIP, arch string) error {
	if probeID == "" {
		return fmt.Errorf("storage: upsert probe heartbeat: empty probe_id")
	}
	var ip, archV sql.NullString
	if lastIP != "" {
		ip = sql.NullString{String: lastIP, Valid: true}
	}
	if arch != "" {
		archV = sql.NullString{String: arch, Valid: true}
	}
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO probes (probe_id, version, started_at, last_seen_at, last_ip, arch)
		 VALUES (?, ?, ?, ?, ?, ?)
		...
		probeID, version, startedAt.UnixMilli(), lastSeenAt.UnixMilli(), ip, archV,
	)
	if err != nil {
...
// SetProbePin flips a probe's fleet-update opt-out. Unknown probe is
// an error — the UI only offers the toggle on registered probes.
func (s *Store) SetProbePin(ctx context.Context, probeID string, pin bool) error {
	res, err := s.db.ExecContext(ctx,
		`UPDATE probes SET pin = ? WHERE probe_id = ?`, pin, probeID)
	if err != nil {
		return fmt.Errorf("storage: set probe pin: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
		return fmt.Errorf("storage: set probe pin: probe %q not registered", probeID)
	}
	return nil
}
...
func (s *Store) ListProbes(ctx context.Context) ([]Probe, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT probe_id, version, started_at, last_seen_at, label, last_ip, arch, pin
	...
	for rows.Next() {
		var p Probe
		var version, label, lastIP, arch sql.NullString
		var startedAt sql.NullInt64
		if err := rows.Scan(&p.ProbeID, &version, &startedAt, &p.LastSeenAt, &label, &lastIP, &arch, &p.Pin); err != nil {
