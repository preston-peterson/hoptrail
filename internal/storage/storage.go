...
type Store struct {
	...
	// for files) — CheckpointWAL shrinks the pool to 1 to quiesce
	// readers and must restore it to exactly this.
	maxConns int
}
...
func Open(path string) (*Store, error) {
	...
	s := &Store{db: db, maxConns: maxConns}
...
func (s *Store) CheckpointWAL(ctx context.Context) (int, error) {
	// QUIESCE BY EXHAUSTING THE POOL: acquire every connection, then
	// checkpoint on one of them. Once all conns are held, nothing
	// in-process can touch SQLite (every reader and writer goes
	// through this pool), so TRUNCATE cannot be blocked — no busy
	// timeouts, no fairness games.
	//
	// Why this shape, hard-won on a live box: TRUNCATE's final phase
	// needs exclusive WAL read-mark slot locks and SQLite acquisition
	// has no fairness, so a dashboard polling every second starves it
	// forever (60s busy timeouts lost 10/10 attempts with 100% of
	// frames already copied). Shrinking the pool via SetMaxOpenConns
	// also fails under sustained demand: database/sql hands freed
	// conns DIRECTLY to waiting requests, bypassing the lowered cap,
	// so the pool never drains ("pool did not drain" 3 sweeps in a
	// row) — and each failed 30s drain was itself a write outage.
	// Acquiring the conns instead rides the same handoff mechanism we
	// were losing to: each conn is handed to us as it frees, queries
	// keep flowing on the rest during acquisition, and the full hold
	// lasts only the reset itself (sub-second — passive checkpoints
	// keep the back-copy current between sweeps).
