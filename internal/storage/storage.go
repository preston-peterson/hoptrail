...
// Store wraps the database and provides the storage layer's lifecycle
// methods. It is safe for concurrent use (database/sql is); the Sink
// type below adds batching on top.
type Store struct {
	db *gatedDB
	// maxConns is the pool bound Open configured (1 for :memory:, 8
	// for files) — CheckpointWAL acquires exactly this many
	// connections to quiesce the pool.
	maxConns int
}

// gatedDB wraps *sql.DB with a writer-preferring gate around every
// query entry point. Exists for exactly one caller: CheckpointWAL.
//
// The WAL truncate needs a zero-reader moment, and 2026-08 taught us
// (three times, in production) that NOTHING below this layer can be
// made to converge on one under continuous load: SQLite's read-mark
// slot locks have no fairness (a 1-second dashboard poll starves the
// checkpointer forever), database/sql's pool handoff is random (the
// acquire-all endgame concentrated all traffic on the last conn and
// then lost the lottery for it — live failure: "quiesce held 7/8
// conns"). sync.RWMutex is the one primitive in the stack with writer
// preference: once Lock() is pending, new RLock() calls block, so
// convergence is guaranteed in the time of the slowest in-flight
// operation instead of probabilistic against an unbounded stream.
//
// The RLock spans only the call itself, not the returned rows'
// lifetime — that looseness is fine because the gate's job is merely
// to stop NEW arrivals; the conn-acquisition step in CheckpointWAL is
// what waits out the stragglers.
type gatedDB struct {
	*sql.DB
	gate sync.RWMutex
}

func (g *gatedDB) QueryContext(ctx context.Context, query string, args ...any) (*sql.Rows, error) {
	g.gate.RLock()
	defer g.gate.RUnlock()
	return g.DB.QueryContext(ctx, query, args...)
}

func (g *gatedDB) QueryRowContext(ctx context.Context, query string, args ...any) *sql.Row {
	g.gate.RLock()
	defer g.gate.RUnlock()
	return g.DB.QueryRowContext(ctx, query, args...)
}

func (g *gatedDB) ExecContext(ctx context.Context, query string, args ...any) (sql.Result, error) {
	g.gate.RLock()
	defer g.gate.RUnlock()
	return g.DB.ExecContext(ctx, query, args...)
}

func (g *gatedDB) Query(query string, args ...any) (*sql.Rows, error) {
	g.gate.RLock()
	defer g.gate.RUnlock()
	return g.DB.Query(query, args...)
}

func (g *gatedDB) QueryRow(query string, args ...any) *sql.Row {
	g.gate.RLock()
	defer g.gate.RUnlock()
	return g.DB.QueryRow(query, args...)
}

func (g *gatedDB) Exec(query string, args ...any) (sql.Result, error) {
	g.gate.RLock()
	defer g.gate.RUnlock()
	return g.DB.Exec(query, args...)
}

func (g *gatedDB) Begin() (*sql.Tx, error) {
	g.gate.RLock()
	defer g.gate.RUnlock()
	return g.DB.Begin()
}

func (g *gatedDB) BeginTx(ctx context.Context, opts *sql.TxOptions) (*sql.Tx, error) {
	g.gate.RLock()
	defer g.gate.RUnlock()
...
func Open(path string) (*Store, error) {
	...
	if path == ":memory:" {
		...
		// Bound the pool (2026-08 WAL incident): database/sql defaults
		// to an UNLIMITED number of connections, and every concurrent
		// reader delays the moment SQLite can reset the WAL. Eight is
		// plenty for the UI's polling plus ingest plus the sink; the
		// real WAL bound is the retention worker's explicit TRUNCATE
		// checkpoint, this just widens the zero-reader gaps it needs.
		// Idle == open so warm connections are reused, not churned.
		maxConns = 8
	...
	s := &Store{db: &gatedDB{DB: db}, maxConns: maxConns}
...
// CheckpointWAL forces a blocking wal_checkpoint(TRUNCATE): every
// frame is copied into the main database file and the -wal file is
// truncated to zero bytes. Returns the number of frames checkpointed.
//
// This exists because SQLite's autocheckpoint is PASSIVE — it can only
// reset the WAL in a moment with zero active readers, and under
// hoptrail's continuous read load (UI polling + ingest + sink commits)
// that moment may never arrive on its own. In the 2026-08 incident the
// WAL grew to 12.5 GiB (3,300× the autocheckpoint threshold) and
// filled the disk with pages that folded into ZERO net database growth
// on shutdown. The retention worker calls this after every sweep — the
// point of maximum churn. TRUNCATE waits on stragglers via the DSN's
// busy_timeout; a busy return is an error so callers log it rather
// than mistaking a skipped reset for a clean one.
func (s *Store) CheckpointWAL(ctx context.Context) (int, error) {
	// QUIESCE IN TWO STAGES — gate, then exhaust the pool.
	//
	// Stage 1: take the writer-preferring gate. From this instant no
	// new query can start (pending Lock blocks new RLocks), and Lock
	// returns once every in-flight call has left the gate. This is
	// the piece with GUARANTEED convergence — the missing property in
	// both prior designs: SQLite's slot locks have no fairness (a 1s
	// dashboard poll starved 60s busy timeouts 10/10), and acquiring
	// conns against open traffic made the pool handoff a lottery that
	// concentrated all remaining traffic on the last conn ("quiesce
	// held 7/8 conns: context deadline exceeded", live, 06:05Z).
	//
	// Stage 2: acquire every pool connection (raw, ungated — we hold
	// the gate). The only competitors left are straggling rows
	// iterations and open transactions from calls that entered before
	// the gate; they finish in ms and their conns come to us, the
	// sole waiter. Holding all conns, nothing in-process can touch
	// SQLite, so TRUNCATE cannot be blocked. Total stall for other
	// traffic = slowest in-flight op + the sub-second reset (passive
	// checkpoints keep the back-copy current between sweeps).
	s.db.gate.Lock()
	defer s.db.gate.Unlock()

	acquireCtx, cancel := context.WithTimeout(ctx, 60*time.Second)
	defer cancel()
	conns := make([]*sql.Conn, 0, s.maxConns)
	defer func() {
		for _, c := range conns {
			_ = c.Close()
		}
	...
	for i := 0; i < s.maxConns; i++ {
		c, err := s.db.DB.Conn(acquireCtx)
		if err != nil {
			return 0, fmt.Errorf("storage: wal_checkpoint: quiesce held %d/%d conns: %w", len(conns), s.maxConns, err)
		}
		conns = append(conns, c)
	}

	var busy, logFrames, checkpointed int
	row := conns[0].QueryRowContext(ctx, "PRAGMA wal_checkpoint(TRUNCATE)")
	if err := row.Scan(&busy, &logFrames, &checkpointed); err != nil {
		return 0, fmt.Errorf("storage: wal_checkpoint: %w", err)
	}
	if busy != 0 {
		return checkpointed, fmt.Errorf("storage: wal_checkpoint: blocked by a concurrent reader/writer (%d of %d frames checkpointed)", checkpointed, logFrames)
	}
	return checkpointed, nil
}

// DB returns the gated database handle the HTTP read handlers query
// through. Deliberately NOT the raw *sql.DB: the handlers' per-second
// polling is exactly the traffic the checkpoint gate must be able to
// hold at the door (see gatedDB).
func (s *Store) DB() *gatedDB { return s.db }
