...
func countSamples(t *testing.T, db *sql.DB) int {
	t.Helper()
	var n int
...
func countRouteChanges(t *testing.T, db *sql.DB) int {
	t.Helper()
	var n int
...
func TestBatchedSink_TickFlushesSmallBatch(t *testing.T) {
	...
	for ttl := uint8(1); ttl <= 3; ttl++ {
		if err := sink.WriteSample(sampleAt(t, ttl, "10.0.0.1", time.Millisecond)); err != nil {
...
func TestBatchedSink_WriteRouteChange(t *testing.T) {
	...
	rc := probe.RouteChange{
		...
		TTL:    3,
		Ts:     time.Now(),
		OldIP:  mustIP(t, "10.0.0.5"),
		NewIP:  mustIP(t, "10.0.0.9"),
	}
	...
	if oldIP != "10.0.0.5" {
		t.Errorf("old_ip = %q, want 10.0.0.5", oldIP)
	}
	if newIP != "10.0.0.9" {
		t.Errorf("new_ip = %q, want 10.0.0.9", newIP)
	}
}
...
func TestSaveAndListBundles_RoundTrip(t *testing.T) {
	...
	time.Sleep(2 * time.Millisecond)
	if err := store.SaveBundle(ctx, "isp-path", tabsOf("192.168.1.1", "cloudflare.com")); err != nil {
		t.Fatalf("save: %v", err)
	}
	...
	if !reflect.DeepEqual(got[0].Targets, []string{"192.168.1.1", "cloudflare.com"}) {
