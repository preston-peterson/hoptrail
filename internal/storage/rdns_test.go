...
// Layout written:
//
//	TTL 1, IP 192.0.2.1   (will be left unresolved in some tests)
//	TTL 2, IP 203.0.113.1
//	TTL 3, IP 8.8.8.8
...
func TestUnresolvedIPs_ReturnsDistinctIPsNotInRDNS(t *testing.T) {
	...
	ips, _, err := store.UnresolvedIPs(context.Background(), 0, 100)
	if err != nil {
...
func TestUnresolvedIPs_ExcludesAlreadyResolved(t *testing.T) {
	...
	if err := store.UpsertRDNS(ctx, "192.0.2.1", ""); err != nil { // NULL row — "no PTR"
		t.Fatalf("UpsertRDNS empty: %v", err)
	}

	ips, _, err := store.UnresolvedIPs(ctx, 0, 100)
	if err != nil {
		t.Fatalf("UnresolvedIPs: %v", err)
	}
	got := map[string]bool{}
	...
	if got["192.0.2.1"] {
		t.Error("192.0.2.1 was resolved with NULL hostname; still counts as attempted, should not appear")
	}
	if !got["203.0.113.1"] || !got["8.8.4.4"] {
		t.Errorf("expected two remaining unresolved IPs (203.0.113.1 and 8.8.4.4), got %v", got)
	}
}

func TestUnresolvedIPs_HonorsLimit(t *testing.T) {
	store := rdnsTestSetup(t)
	ips, _, err := store.UnresolvedIPs(context.Background(), 0, 2)
	if err != nil {
		t.Fatalf("UnresolvedIPs: %v", err)
	}
	if len(ips) != 2 {
...
func TestUpsertRDNS_EmptyHostnameStoredAsNULL(t *testing.T) {
	...
	if err := store.UpsertRDNS(ctx, "203.0.113.1", ""); err != nil {
		t.Fatalf("UpsertRDNS: %v", err)
	}

	// Verify via direct query: the row exists, hostname is NULL.
	var hostname any
	err := store.db.QueryRow("SELECT hostname FROM rdns WHERE ip = ?", "203.0.113.1").Scan(&hostname)
	if err != nil {
...
func TestLookupHostnames_ReturnsOnlyResolvedNames(t *testing.T) {
	...
	if err := store.UpsertRDNS(ctx, "203.0.113.1", ""); err != nil { // no PTR
		t.Fatal(err)
	}

	// Query for all four — only the two with hostnames should come back.
	names, err := store.LookupHostnames(ctx, []string{
		"192.0.2.1", // never resolved → absent from map
		"203.0.113.1",    // NULL hostname → absent from map
