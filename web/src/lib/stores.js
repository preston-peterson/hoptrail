...
// settingsOpen drives the settings overlay (gear icon). settingsTab
// selects which category page is showing — the overlay is organized
// as one page per category with a persistent left nav, so callers
// can deep-link ("Enable in settings" → the Bandwidth page).
export const settingsOpen = writable(false)
export const settingsTab = writable('bandwidth')

/** Opens the settings overlay, optionally landing on a specific page. */
...
// soundConfig mirrors GET /api/alerts/sound — server-shared policy
// (master + per-event toggles). Whether THIS browser can actually
// play is the autoplay arming in lib/sound.js, a separate concern.
export const soundConfig = writable(null)

export async function setSoundConfig(patch) {
  const stored = await patchSoundConfig(patch)
  soundConfig.set(stored)
  return stored
}

// lastSoundedId is per-browser (localStorage): which alert_history id
// this browser has already sounded for. Distinct from alertsSeenId
// (the bell badge) — viewing the history shouldn't silence future
// sounds, and a sound shouldn't clear the badge. null until the first
// status tick: the first sight of latest_history_id is recorded
// WITHOUT sounding, so a fresh browser doesn't beep about old history.
let lastSoundedId = (() => {
  const v = localStorage.getItem('hoptrail-alerts-sounded')
  return v == null ? null : Number(v)
})()

function recordSounded(id) {
  lastSoundedId = id
  localStorage.setItem('hoptrail-alerts-sounded', String(id))
}

// initStores can re-run (SPA re-init); the status subscription must
// not stack.
let soundWatcherInstalled = false

// watchAlertSounds rides the existing 30s status poll: when
// latest_history_id advances, fetch the new entries, filter by the
// per-event toggles, and play one tone for the batch — the raise tone
// if anything raised, else the all-clear.
async function onStatusForSound(s) {
  if (!s) return
  const latest = s.alerts?.latest_history_id ?? 0
  if (lastSoundedId == null) {
    recordSounded(latest)
    return
  }
  if (latest <= lastSoundedId) return
  const prev = lastSoundedId
  recordSounded(latest) // before any await — no double-fire on slow fetches
  const cfg = get(soundConfig)
  if (!cfg?.enabled) return
  try {
    const hist = await fetchAlertHistory(50)
    const fresh = (hist.entries ?? []).filter(
      (e) => e.id > prev && cfg.events?.[e.event_type]
    )
    if (!fresh.length) return
    if (fresh.some((e) => e.kind === 'alert')) playRaise()
    else playRecover()
  } catch {
    // Status keeps polling; a missed tone is not worth surfacing.
  }
}

// envHealth distills the aggregate into one light: 'red' (probe
// offline or active incident or sudoers drift or disk critical),
// 'amber' (derate, alert queue backing up, or disk low), 'green'
// otherwise. Disk health is read straight from the capacity
// classification so a full disk lights the dot even when alerting is
// off. The dot is a summary, not a diagnosis — the overlay has details.
export const envHealth = derived(statusStore, ($s) => {
  if (!$s) return 'unknown'
  const probeDown = ($s.probes ?? []).some((p) => !p.online)
  if (probeDown || ($s.alerts?.active_incidents ?? 0) > 0 || $s.update?.sudoers_ok === false || $s.database?.health === 'critical') return 'red'
  if ($s.bandwidth?.derate || ($s.alerts?.queue_depth ?? 0) > 0 || $s.database?.health === 'warn') return 'amber'
  return 'green'
})
...
// refreshProbes does one immediate fetch — used after a probe-update
// command so the new "updating" state shows without waiting out the
// 10s poll (step-176).
...
export function initStores() {
  ...
  // #20: audible alerts. Policy is server-shared; fetch once at
  // startup (the settings panel refreshes it on open). The watcher
  // rides the status poll; installAutoArm lets this browser start
  // audio on its first natural interaction (autoplay rule).
  fetchSoundConfig()
    .then((cfg) => soundConfig.set(cfg))
    .catch(() => {})
  installAutoArm()
  if (!soundWatcherInstalled) {
    soundWatcherInstalled = true
    statusStore.subscribe(onStatusForSound)
  }
