...
// ---------- central-driven probe updates (#22) ----------

/** Commands a central-driven update for one probe. Returns the update row. */
export async function commandProbeUpdate(probeId) {
  const res = await fetch(BASE + '/probes/' + encodeURIComponent(probeId) + '/update', { method: 'POST' })
  if (!res.ok) {
    const msg = (await res.text()).trim() || `HTTP ${res.status}`
    throw new Error(msg)
  }
...
/** Cancels a still-pending update command. 409 once the probe is applying. */
export async function cancelProbeUpdate(probeId) {
  const res = await fetch(BASE + '/probes/' + encodeURIComponent(probeId) + '/update', { method: 'DELETE' })
  if (!res.ok) {
    const msg = (await res.text()).trim() || `HTTP ${res.status}`
    throw new Error(msg)
  }
...
/** Sets a probe's fleet-update opt-out. */
export async function setProbePin(probeId, pin) {
  const res = await fetch(BASE + '/probes/' + encodeURIComponent(probeId), {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
/** Starts the staggered update-all rollout. */
export async function startProbesUpdateAll() {
  const res = await fetch(BASE + '/probes/update-all', { method: 'POST' })
  if (!res.ok) {
    const msg = (await res.text()).trim() || `HTTP ${res.status}`
    throw new Error(msg)
  }
...
/** Rollout status: { running, current, done, failed }. */
export function fetchProbesUpdateAll() {
  return getJSON('/probes/update-all')
}
...
// ---------- release-update mode (#11) ----------

/** Contacts GitHub now. Returns {checked_at, latest_version,
 *  update_available, release_url, error} — a failed check is a 200
 *  with the error recorded, so the UI can show it inline. */
export async function checkForUpdates() {
  const res = await fetch(BASE + '/update/check', { method: 'POST' })
  if (!res.ok) {
    const msg = (await res.text()).trim() || `HTTP ${res.status}`
    throw new Error(msg)
  }
...
/** Downloads + sha256-verifies the latest release binary into the
 *  staged slot. Returns the staged info (same shape as upload). */
...
/** Persists update settings, currently {check_interval: off|daily|weekly|monthly}. */
export async function patchUpdateSettings(patch) {
...
/** Audible-alert policy (#20): { enabled, events: {type: bool} }. */
export function fetchSoundConfig() {
  return getJSON('/alerts/sound')
}

/** PATCHes the sound policy; omitted event keys keep their values. */
export async function patchSoundConfig(cfg) {
  const res = await fetch(BASE + '/alerts/sound', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(cfg),
  })
