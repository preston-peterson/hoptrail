...
function inline(s) {
  ...
  s = s.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_, text, url) => {
    ...
    // SECURITY (step-170, audit #15): only allow safe URL schemes.
    // The renderer's output is injected via {@html}; a javascript:,
    // data:, or vbscript: href would execute on click. Allow only
    // http/https/mailto and relative (#, /, ./) URLs; anything else
    // renders as inert text. esc() already neutralized quotes/angles
    // in `url`, so a scheme check is sufficient here.
