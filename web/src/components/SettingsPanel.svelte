<script>
  ...
  import { settingsOpen, refreshBandwidthConfig, retentionDays, docsOpen, theme, setTheme } from '../lib/stores.js'
  ...
  // Collapse-all / expand-all (#31). If any section is open, the
  // toggle collapses everything; otherwise it expands everything.
  $: anyOpen = bandwidthOpen || probesOpen || alertsOpen || retentionOpen || systemOpen || updatesOpen || aboutOpen
  function toggleAll() {
    const next = !anyOpen
    bandwidthOpen = probesOpen = alertsOpen = retentionOpen = systemOpen = updatesOpen = aboutOpen = next
  }
  ...
  <aside class="panel" aria-label="Settings">
    <header>
      ...
      <div class="head-actions">
        <button class="head-btn" on:click={toggleAll} title={anyOpen ? 'collapse all sections' : 'expand all sections'}>
    ...
    <!-- Theme: pinned at the foot of the gear menu as a segmented
         control (the conventional spot). Per-browser; no server round
         trip. Sole theme control — not duplicated elsewhere. -->
...
<style>
  ...
  .head-btn:hover { color: var(--fg); border-color: var(--fg-subtle); }
  .close {
  ...
  /* Theme footer — pinned segmented control, mt:auto pushes it to the
