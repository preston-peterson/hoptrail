<script>
  ...
  import { derateStatus, bandwidthConfig, versionStore, settingsOpen, refreshBandwidthConfig } from '../lib/stores.js'
    ...
    <span class="text">
      ...
      <span class="detail">
        ...
        {#if cfg.capability.available}
          <button class="linkish" on:click={() => settingsOpen.set(true)}>Enable in settings</button>
        {:else}
          <button class="linkish" on:click={() => settingsOpen.set(true)}>Install it from settings</button>
        {/if}
      </span>
    </span>
    <span class="actions">
