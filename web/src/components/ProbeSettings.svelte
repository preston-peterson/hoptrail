<script>
  ...
  import { probesStore, versionStore } from '../lib/stores.js'
  import {
    fetchProbeTokens,
    createProbeToken,
    revokeProbeToken,
  ...
  // Probe update guide (#21): which probe's popout is open.
  ...
  function oneLiner(m) {
    ...
    return (
      'curl -fsSL https://get.hoptrail.net | ' +
      ...
      {#each remoteProbes as p (p.probe_id)}
        <li>
          ...
          {#if p.outdated}
            ...
            <button class="update" on:click={() => (updateGuideProbe = p)}>Update</button>
          {/if}
