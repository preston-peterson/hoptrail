<script>
  ...
  // The install command for the remote host. The browser already
  // knows how it reaches this central — location is the right
  // default for central_url (overridable by editing the pasted line).
  function oneLiner(m) {
    const central = `${location.protocol}//${location.host}`
    return (
      'git clone https://github.com/preston-peterson/hoptrail.git && ' +
      `cd hoptrail && ./install.sh --probe --id ${m.name} --central ${central} --token ${m.token}`
    )
  }
