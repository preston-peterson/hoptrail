<script>
  ...
  let uploading = false
  let checking = false
  // applying: false → 'downloading' → 'applying' → 'restarting' → 'done'
  let applying = false
  ...
  // One-click release update: download + verify + stage, then apply +
  // restart — no separate "now scroll down and apply" step (operator
  // feedback; matches the sibling project's single-button flow).
  async function downloadAndApply() {
    error = ''
    newVersion = status?.last_check?.latest_version ?? ''
    applying = 'downloading'
    try {
      await downloadUpdate() // ~20s: fetch from GitHub, sha256-verify, stage
      applying = 'applying'
      const res = await applyUpdate()
      newVersion = res.new_version || newVersion
      applying = 'restarting'
      watchRestart()
    } catch (err) {
      applying = false
...
<div class="updates">
  ...
  {:else}
    ...
    {#if applying === 'downloading'}
      <p class="busy">Downloading v{newVersion} from GitHub (verifying)… this takes ~20s.</p>
    {:else if applying === 'applying'}
      <p class="busy">Applying v{newVersion}…</p>
    ...
    {:else}
      {#if status.release_available}
        <div class="release">
          ...
          {/if}
          <div class="actions">
            {#if status.last_check?.update_available && status.sudoers.ok}
              <!-- One click: download + verify + stage + apply + restart. -->
              <button class="apply primary" on:click={downloadAndApply}>
                Download &amp; apply v{status.last_check.latest_version}
              </button>
            {/if}
            <button class="check" disabled={checking} on:click={check}>
      ...
      <!-- Manual / self-built path — secondary. A staged binary (from a
           hand upload, or a release download whose apply failed) shows
           its own Apply here. -->
      <details class="manual">
        <summary>Update from a local binary instead</summary>
        ...
        <div class="actions">
          <label class="upload-btn" class:disabled={uploading}>
            {uploading ? 'Uploading…' : status.staged.present ? 'Upload a different binary' : 'Upload a binary'}
        ...
        <p class="muted small">
          For self-built binaries. Applies in place: the current binary
          is backed up, the new one swapped in, <code>cap_net_raw</code>
          re-applied, and the service restarted.
        </p>
      </details>
    {/if}
...
<style>
  ...
  .apply.primary:hover:not(:disabled) {
    color: #fff;
  ...
  .manual > :global(*:not(summary)) { margin-top: 6px; }
</style>
