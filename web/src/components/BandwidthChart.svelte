<script>
  ...
  import { bandwidthSamples, bandwidthWindow, bandwidthChartWindow, setBandwidthChartWindow, BW_WINDOWS, derateStatus, bandwidthConfig, settingsOpen, bandwidthSectionVisible } from '../lib/stores.js'
    ...
    {:else}
      <div class="empty muted">
        ...
        {:else if cfg.enabled}
          ...
          <button class="linkish" on:click={() => settingsOpen.set(true)}>run one now</button>
        {:else}
          bandwidth monitoring is off —
          <button class="linkish" on:click={() => settingsOpen.set(true)}>enable it in settings</button>
        {/if}
      </div>
    {/if}
  </section>
