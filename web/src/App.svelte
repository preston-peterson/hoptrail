<script>
  ...
  // Per-section heights (step-164, #23): operator-dragged px values
  // win over the CSS defaults. liveHeights overrides during a drag so
  // the section tracks the cursor without a PATCH per mousemove.
  let liveHeights = null
  $: effHeights = liveHeights ?? layout.heights ?? {}
  $: gridRows = visibleMain
    .map((id) =>
      layout.collapsed[id] ? 'auto'
      : effHeights[id] ? `${effHeights[id]}px`
      : SECTION_ROWS[id])
    .join(' ')
  ...
  // ---- section height handle (step-164: the splitter's horizontal
  // twin — drag a section's bottom edge, persist on mouseup) ----
  function startSectionResize(e) {
    const { id, startY, startHeight } = e.detail
    const onMove = (ev) => {
      const h = Math.max(120, Math.min(1600, Math.round(startHeight + ev.clientY - startY)))
      liveHeights = { ...(layout.heights ?? {}), [id]: h }
    }
    const onUp = () => {
      window.removeEventListener('mousemove', onMove)
      window.removeEventListener('mouseup', onUp)
      if (liveHeights && liveHeights[id] !== (layout.heights ?? {})[id]) {
        saveSectionLayout({ ...layout, heights: liveHeights })
      }
      liveHeights = null
    }
    window.addEventListener('mousemove', onMove)
  ...
  function resetSectionHeight(id) {
    const heights = { ...(layout.heights ?? {}) }
    if (!(id in heights)) return
    delete heights[id]
    saveSectionLayout({ ...layout, heights })
  }
...
>
  {#each visibleMain as id (id)}
    <DashboardSection
      ...
      on:resizestart={startSectionResize}
      on:resizereset={(e) => resetSectionHeight(e.detail.id)}
    >
