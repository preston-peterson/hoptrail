...
**The install one-liner now points at GitHub directly.** Every install
and probe-update command — in the README, the docs, and the commands
the UI generates for you to paste — now fetches
`raw.githubusercontent.com/preston-peterson/hoptrail/main/get.sh`
instead of going through the `get.hoptrail.net` redirect. The redirect
keeps working for anything that still references it.

## [0.7.6] — 2026-08-14

**The WAL checkpoint can no longer be starved — and it runs every ten
minutes.** Overnight monitoring after 0.7.5 caught the truncate losing
once more, a third starvation variant: while it collected the
connection pool, the connection handoff's random ordering let it hold
7 of 8 and lose the race for the last one indefinitely. The checkpoint
now closes a writer-preferring gate first — new queries pause (they
queue, nothing fails), in-flight work drains, and only then does it
take the pool, which makes convergence guaranteed rather than
probabilistic. The gate is also what ends the "database is locked"
bursts: measurements showed those errors clustered entirely around the
old checkpoint's own failed quiesce attempts, not around database
load. The truncate additionally runs on a ten-minute cadence instead
of hourly, so a missed checkpoint costs minutes of log growth rather
than a full hour's, and the log's steady peak drops from ~215 MB to
~35 MB.

**Hostname lookups no longer rescan the whole database every minute.**
The reverse-DNS worker's poll asked "which IPs have I never tried?"
with a query that read every sample row — tens of millions on a
long-running install — even when the answer was "none", holding a
database connection for the whole scan, and the scan got slower as the
write-ahead log grew. The worker now remembers how far it has looked
and scans only rows added since, making the routine poll effectively
free. (That standing scan is also the prime suspect for the overnight
checkpoint failures stalling at exactly 7 of 8 connections.)

**Removing a probe (or target) now clears its active alerts.** An
incident could outlive its subject: the alert engine only clears an
incident when it re-evaluates the subject and finds it healthy, and a
probe removed from the registry is never evaluated again — so its
"probe offline" incident sat on the status card forever, with no way
to dismiss it. Deleting a probe or target now clears that subject's
incidents immediately, and the engine sweeps up any orphaned incident
each evaluation pass as a backstop. Cleared incidents get a "cleared"
entry in the alert history — deliberately not a "recovered"
notification, since nothing recovered.

**The retention startup log line now reports the policy actually in
effect.** It used to print the config-file value even when the
settings-panel override (stored in the database) was what the sweeps
actually used — a 7-vs-30-day discrepancy that misled a dataset-sizing
estimate by 4×. The banner now shows the effective retention days and
the new checkpoint cadence.

## [0.7.5] — 2026-08-13

**A failing store can no longer turn the bandwidth schedule into a
retry storm.** A live incident: with the disk full, the daily
speedtest's result could not be stored, so the scheduler never saw the
slot as served and its catch-up path re-ran a full line-saturating test
every 5 seconds — ~700 attempts per hour — until the service was
stopped. The scheduler now keeps an in-memory record of its last
attempt: catch-up fires at most once per slot per process, and
interval-mode cadence is paced by the attempt when the stored sample
can't advance. Even with storage completely broken, you get at most one
test per scheduled slot. Catch-up also now waits four minutes after
startup instead of five seconds, clearing the startup retention sweep
it used to collide with — both fire on exactly the restarts catch-up
exists for, and the collision reliably ate the catch-up's result.

**The database write-ahead log is now actively bounded.** The same
incident's root cause: under hoptrail's continuous read load (the UI
polling, probes ingesting), SQLite's passive auto-checkpoint never
finds the idle moment it needs to reset the WAL, so the log grew
without limit — 12.5 GiB of rewritten pages that folded into zero
database growth — and filled the disk. The retention worker now forces
a blocking truncate-checkpoint after every hourly sweep, retention
deletes run in bounded batches instead of one giant transaction, and
the connection pool is capped instead of unlimited. The checkpoint
retries on a busy collision rather than conceding a full hour of
unbounded growth to one unlucky moment.

**A failing store can no longer turn one alert into a raise storm.**
Same incident, fourth finding: when the disk is full the alert engine
can't persist "this incident is already active", so every 15-second
evaluation re-raised the same incident as new — 240 attempts/hour from
one subject, saturating the global rate limit, starving every other
alert (including the disk-capacity alert that mattered), and spamming
the log hard enough to rotate the journal away. The per-incident
cooldown is now recorded when a raise is *attempted* rather than when
it's delivered, bounding re-raises to one per cooldown window per
subject no matter how broken storage is.

**New emergency kill switch: `HOPTRAIL_NO_SPEEDTEST`.** Bandwidth
settings live in the database, which means a full disk locks you out of
the off toggle exactly when you need it. Setting this environment
variable on the service (any non-empty value) stops all speedtests —
scheduled, catch-up, and manual — regardless of settings, while
everything else keeps running. See the operations guide.

**The status page now names its active alerts.** The Alerts card used
to show a bare count — "active incidents: 1" — leaving you to hunt for
what actually fired. Each active incident is now listed right on the
card (what fired and on which target or probe, and since when), and
clicking it opens the alert history for the full story.
*Under the hood:* `/api/status` gains an `alerts.active` array
(`event`, `subject`, `since`); the handler already iterated the alert
states for the count and now collects them. Older servers without the
field render exactly as before.

**Security dependency updates.** `golang.org/x/net` (the library under
the ICMP probe engine) updated 0.27 → 0.57, covering three published
advisories against older versions; the `postcss` build tool updated
past its patched version. Note for source builds: the minimum Go
version is now 1.25 (prebuilt binaries and the installer one-liner are
unaffected).
