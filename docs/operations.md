...
```bash
curl -fsSL https://raw.githubusercontent.com/preston-peterson/hoptrail/main/get.sh | bash -s -- --probe
```
...
- WAL mode means `-wal`/`-shm` sidecars appear next to the file while
  the daemon runs; that's normal. The retention worker forcibly
  checkpoints and truncates the `-wal` file after every hourly sweep,
  so it should stay small; a `-wal` file that keeps growing into the
  gigabytes is a bug — check the logs for `retention: wal checkpoint`
  warnings.
...
## Emergency: disabling speedtests out-of-band

Bandwidth settings (including the on/off toggle) live in the database
and are edited through the UI. That has one sharp edge: if storage is
failing — a full disk, most likely — the toggle can't be written, at
the exact moment you may want scheduled tests to stop.

The escape hatch is an environment variable checked at startup:

```
sudo systemctl edit hoptrail
# in the editor, add:
[Service]
Environment=HOPTRAIL_NO_SPEEDTEST=1

sudo systemctl restart hoptrail
```

With `HOPTRAIL_NO_SPEEDTEST` set (any non-empty value), no speedtest
runs — scheduled, catch-up, or manual — regardless of what the settings
say. Probing, the UI, and everything else keep working. The daemon logs
a warning at startup while the switch is active, and manual test
attempts from the UI return an error naming the variable. Remove the
drop-in and restart to re-enable.
