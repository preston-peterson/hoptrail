...
**outdated**) and the panel's **Update** button walks through it —
one pasted command on the probe host:
...
re-enter); the service restarts on the new binary and the probe
reappears with its new version within one heartbeat. The flag
compares release versions, so a central running a dev build doesn't
nag probes on the same release.
