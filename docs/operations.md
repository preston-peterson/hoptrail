...
**From the web UI** (the easy path): gear icon → **Updates** → upload
a hoptrail binary → **Apply**. The daemon backs up the old binary,
swaps in the new one, re-applies `cap_net_raw`, and restarts itself
through the sudoers rule; the page reloads on the new version.
Updating straight from GitHub releases lands once the project
publishes.
