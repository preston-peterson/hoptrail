...
```bash
git clone https://github.com/preston-peterson/hoptrail.git && cd hoptrail && ./install.sh --probe --id site-east --central http://192.0.2.10:8080 --token <minted-token>
```

Copy it, paste it on the remote Linux host (a regular user with sudo),
done. The token is shown only once — if it's lost, revoke it and add
