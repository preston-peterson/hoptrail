...
```bash
curl -fsSL https://raw.githubusercontent.com/preston-peterson/hoptrail/main/get.sh | bash -s -- --probe --id site-east --central http://192.0.2.10:8080 --token <minted-token>
```
...
Probes report their version (and architecture) on every heartbeat.
When one falls behind the central's release, its row in the Probes
panel grows an **update available** chip and an **Update** button —
click it and the central downloads the release binary for that
probe's architecture, verifies it, and delivers it over the same
authenticated channel the heartbeats use; the probe re-verifies the
sha256, swaps itself with a backup kept, and restarts. The row shows
the progress (updating → vX.Y.Z…), and the probe reappears current
within a heartbeat of coming back.

**Update all** runs the same thing across every outdated probe, one
at a time — a failure stops the line so a bad build never reaches the
whole fleet. The 📌 pin on a row excludes that probe from fleet
updates (its own Update button still works). Failures land in the
alert history with the reason; a pending command for an offline probe
simply waits until the probe next heartbeats, and can be canceled
until the probe acknowledges it.

The one exception: a probe running a version that predates this
feature can't receive the command — its button says **How to update**
and shows the one-paste manual path instead. After that single manual
update, it's buttons from then on.
