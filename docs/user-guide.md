...
The one-liner — downloads the latest release's prebuilt binary for
your architecture (sha256-verified) and runs the installer, no build
tools needed:
...
the dock stays pinned.

Sections resize too: a drag bar between the dock and the main pane
sets the split, and each section's **bottom edge** is a height handle —
drag it to shorten or grow that section (the charts follow), or
double-click it to return to the natural height. A section made
shorter than its content scrolls inside its bounds. One layout — order,
dock, widths, heights, collapses — saved server-side, shared by every
browser.
...
The dashboard can also **ring in the browser**: a sound master switch
in the Alerts section, with a per-event toggle under it for each alert
type. Raises play an attention tone; recoveries play a softer
all-clear. The selection is shared by every browser, but a browser can
only ring after you've clicked somewhere in the page once — a browser
autoplay rule, not a hoptrail setting; the two ♪ preview buttons in
the panel let you hear both tones (and count as that click).
