...
Even though the API is unauthenticated, the operator's browser sits on
the same trusted LAN, so a malicious web page could otherwise forge
state-changing requests at hoptrail in the background (CSRF), and a
hostile domain could try to reach it via DNS rebinding. Hoptrail
defends against both: every mutating request must be same-origin and
carry a custom header a cross-origin page cannot set without a CORS
preflight the daemon refuses, and the `Host` header is checked against
an allowlist (loopback, bare IP addresses, and single-label/`.local`
intranet names are always accepted; add reverse-proxy hostnames or
public FQDNs to `allowed_hosts` in the config). Read-only `GET`
endpoints and the bearer-token probe-ingest surface are exempt by
design. If you put hoptrail behind a reverse proxy under a registered
domain, add that hostname to `allowed_hosts` or the UI's write actions
will be refused.
