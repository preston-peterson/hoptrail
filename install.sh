...
# 0700: the data dir holds the SQLite DB with plaintext bearer tokens
# (security audit #16). Owner-only keeps other local users out.
sudo install -d -m 0700 -o "$USER" -g "$USER" "$DATA_DIR"
echo -e "  ${GREEN}✓${RESET} Data directory ${DATA_DIR} (owned by ${USER}, mode 0700)"
...
TimeoutStopSec=10
# Security audit #16: files the daemon creates (the SQLite DB holding
# bearer tokens, its WAL) default to owner-only.
UMask=0077
